...
def run(mode, fp16):
    ...
    p.exp_dir=ROOT/'integration_exp'/f'{mode}-fp{16 if fp16 else 32}'
    ...
    optimizer=d.base.ScaledAdam(d.base.get_parameter_groups_with_lrs(model,lr=.001,include_names=True),lr=.001,clipping_scale=2.)
    ...
    before=model.joiner.output_linear.weight.detach().clone()
    proj_before=model.kd_proj.weight.detach().clone() if hasattr(model,'kd_proj') else None
    d.base.train_one_epoch(params=p,model=model,optimizer=optimizer,scheduler=scheduler,
        sp=sp,train_dl=[batch]*4,valid_dl=[batch],scaler=scaler,model_avg=avg)
    assert p.batch_idx_train==4
    assert torch.isfinite(torch.tensor(p.train_loss))
    assert not torch.equal(before,model.joiner.output_linear.weight)
    if proj_before is not None: assert not torch.equal(proj_before,model.kd_proj.weight)
    if mode!='none':
        assert not model._teacher.training
        assert all(t.grad is None and not t.requires_grad for t in model._teacher.parameters())
        expected=torch.load(p.teacher_checkpoint,weights_only=True)['model']
        assert all(torch.equal(v.cpu(),expected[k]) for k,v in model._teacher.state_dict().items())
    else: assert model._teacher is None
    model.eval()
    val,info=d.compute_loss_distill(p,model,sp,batch,False)
    assert not val.requires_grad and torch.isfinite(val)
    if mode in ('encoder','none'): assert info['kd_logit_loss']==0
    if mode in ('logit','none'): assert info['kd_enc_loss']==0
    path=p.exp_dir/'checkpoint.pt'
    save_checkpoint(path,model,model_avg=avg,optimizer=optimizer,scheduler=scheduler,scaler=scaler,params=p)
    saved=torch.load(path,weights_only=False)
    assert not any('_teacher' in k for k in saved['model'])
    fresh=d.get_model_with_distill(p).cuda().eval()
    load_checkpoint(path,fresh,strict=True)
    val2,_=d.compute_loss_distill(p,fresh,sp,batch,False)
    torch.testing.assert_close(val,val2)
    print(f'PASS mode={mode} fp16={fp16}: 4 epoch-loop updates, validation, frozen teacher, averaging, checkpoint model reload; val={val.item():.4f}',flush=True)

if __name__=='__main__':
    for mode,fp16 in [('encoder',False),('encoder',True),('both',True),('logit',False),('none',False)]:
        run(mode,fp16)
